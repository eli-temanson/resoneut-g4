...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if(reader->GetBeamZ() == 0) {
    if(reader->GetBeamA() == 1) {
    ...
    G4ParticleDefinition* particle = 
      particleTable->GetIonTable()->GetIon(reader->GetBeamZ(),reader->GetBeamA(),0.0);
    pParticleGun->SetParticleDefinition(particle);
  }
  ...
  pParticleGun->SetParticlePosition(
    G4ThreeVector(
      G4RandGauss::shoot(reader->GetBeamPosX()*cm,1.0*cm),
      G4RandGauss::shoot(reader->GetBeamPosY()*cm,1.0*cm),
      reader->GetBeamPosZ()*cm));

  pParticleGun->SetParticleMomentumDirection(
    G4ThreeVector(
      reader->GetBeamPx(),
      reader->GetBeamPy(),
      reader->GetBeamPz()));
