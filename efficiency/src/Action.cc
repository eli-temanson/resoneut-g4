...
void EventAction::EndOfEventAction(const G4Event* event) {
  ...
  if(scintHC->entries() > 0) {
    for(G4int i = 1; i < scintHC->entries(); i++) {
      ...
      //if((*scintHC)[i]->GetEnergy() / MeV >= n_threshold && (*scintHC)[i]->GetTargetName() != "C12" && (*scintHC)[i]->GetParticle()->GetParticleName() != "proton"){

        scintEtot += (*scintHC)[i]->GetEnergy() / MeV;
  ...
  if(scintHC_thin->entries() > 0) {
    for(G4int i = 1; i < scintHC_thin->entries(); i++) {
      ...
      // if((*scintHC_thin)[i]->GetEnergy() / MeV >= n_threshold && (*scintHC_thin)[i]->GetTargetName() != "C12" && (*scintHC_thin)[i]->GetParticle()->GetParticleName() != "proton"){
